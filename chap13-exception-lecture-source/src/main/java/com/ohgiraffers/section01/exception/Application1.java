...
public class Application1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        /*목차. 1. throws로 예외 처리 책임을 위임*/
        ExceptionTest et = new ExceptionTest();

        //상품 가격이 10,000원, 잔고가 50,000원일 경우
        et.checkEnoughMoney(10000,50000);   //정상 동작

        //상품 가격이 50,000원, 잔고가 10,000원일 경우
        et.checkEnoughMoney(50000,10000);   //

        /*설명.
        * 예외처리가 강제화된 메서드를 호출하는 경우, 반드시 예외처리를 해야한다.
        * 그 중, 첫 번째 방법인 throws를 이용해 예외를 처리해보았다.
        * main() 메서드에서 checkEnoughMoney() 메서드를 호출하는데, checkEnoughMoney()는 예외를 처리해야하는 메서드다.
        * 따라서 호출부인 main() 메서드 측에서 이 예외를 처리(핸들링) 해야하는 강제성이 생기게 된다.
        * main()메서드의 헤드부분에 checkEnoughMony() 메서드처럼 throws Exception을 추가하면
        * 컴파일에러는 사라지게 된다.*/
