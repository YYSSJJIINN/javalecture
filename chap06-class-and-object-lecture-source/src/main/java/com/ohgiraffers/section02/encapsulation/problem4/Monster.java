package com.ohgiraffers.section02.encapsulation.problem4;

public class Monster {

    /*설명.
    * Monster클래스의 필드를 private으로 만들고 객체를 생성ㅎ나 뒤에 필드에 직접 접근하면 컴파일 에러가 발생한다.
    * 애초에 클래스 설계 상으로 접근을 허용하지 않았기 때문에 직접 접근할 수 없다는 의미다.
    * 즉, 메서드를 통해 간접 접근을 하도록 문법적으로 강제화한 것이다.
    * */

    /* 필기.
     *  접근제한자(Access modifier)
     *  클래스 혹은 클래스의 멤버에 참조연산자로 접근할 수 있는 범위를 제한하기 위한 키워드이다.
     *  1. public : 모든 패키지에 접근 허용.
     *  2. protected : 동일 패키지에 접근 허용. 단, 상속관계에 있는 경우 다른 패키지에서도 접근 가능
     *  3. default : 동일 패키지에서만 접근 허용. (작성하지 않는 것이 default)
     *  4. private : 해당 클래스 내부에서만 접근 허용.
     *
     * 필기.
     *  위의 네 가지 접근제한자는 클래스의 멤버(필드, 메소드)에 모두 사용 가능하다.
     *  단, 클래스 선언 시 사용하는 접근제한자는 public과 default만 사용 가능하다.
     * */
    private String kinds;     //private 접근 제한자 때문에 컴파일 에러 발생.
    private int hp;           //private 접근 제한자 때문에 컴파일 에러 발생.

    public void setHp(int hp) {
        if(hp > 0)
            this.hp = hp;
        else
            this.hp = 0;
    }

    public void setKinds(String kinds) {
        this.kinds = kinds;
    }

    public String getInfo() {
        return "Monster{" +
                "kinds='" + kinds + '\'' +
                ", hp=" + hp +
                '}';
    }
}
