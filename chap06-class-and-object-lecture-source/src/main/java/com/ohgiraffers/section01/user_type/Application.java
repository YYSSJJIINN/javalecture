...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < hobbies.length; i++) {       //hobbies.length는 3개다. 3회반복.
            //for문 사용해서 hobbies 직접 출력하기
            System.out.print(hobbies[i] + " ");       //hobbies의 i번째 인덱스 출력하기
        }
        System.out.print("]\n");        //개행문자이고, 저렇게 입력하면 엔터가 들어간다.
         ...
         * */
        Member member = new Member();       //이제 변수라고 안하고 인스턴스라고 칭하기. new연산자로 자료값 들어왔으니까.
        ...
        /* 필기.
         *  필드에 접근하기 위해서는 레퍼런스변수명(member).필드명(id pwd등등) 으로 접근한다.
         *  '.'은 참조연산자 라고 하는데, 레퍼런스 변수가 참고하고 있는 주소로 접근한다는 의미를 가진다.
         *  각 공간은 필드명으로 접근한다. (배열[]은 인덱스index[int]로 접근, 객체{}는 필드명으로 접근이다)
         * */
        ...
        /*목차. 2-3. 필드에 접근(.)해서 기존에 우리가 알던 변수 사용법 처럼 사용할 수 있다.*/
        member.id = "user02";
        member.pwd = "pass02";
        member.name = "유관순";
        member.age = 20;
        member.gender = '여';
        member.hobbies = new String[] {"축구", "볼링", "테니스"};
        ...
        for(int i = 0; i <member.hobbies.length; i++) {
            System.out.print(member.hobbies[i] + " ");      //" "의 역할은 띄어쓰기 용도임. 깔끔하게 보이기 용이지 필수는 아니다.
        }
        System.out.println();
