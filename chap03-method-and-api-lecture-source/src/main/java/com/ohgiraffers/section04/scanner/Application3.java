...
public class Application3 {
    ...
    public static void main(String[] args) {
        ...
        //"안녕하세요."와 "안녕하세요, 반갑습니다."를 각각 입력해서 두 번 테스트해본다.
        String str1 = sc.next();
        sc.nextLine();  //개행문자(enter)를 실행해서 버퍼를 비워주는 명령어
        ...
        /* 목차.2. 정수, 실수, 논리값 입력 후 next()로 문자열 입력받을 때*/
        System.out.println("-------------------- 목차 2. --------------------");
        System.out.print("숫자(정수)를 입력해주세요. :");

        int num2 = sc.nextInt();
        sc.nextLine();      //이걸 sc.nextInt()이후에 아무곳에나 붙여줘야 줄이 바뀌어 출력된다.
        System.out.println("num2 = " + num2);

        System.out.print("(공백이 존재하는)문자열을 입력해주세요 :");

        String str2 = sc.nextLine();
        System.out.println("str2 = " + str2);

        /* 설명.
         *  입력받을 시 nextLine()을 호출한 것이 아직 행을 바꾸지 않고(엔터키) 한 라인을 읽어드린다고 판단해서
         * (문자열을 입력해주세요 : str2 로 출력됐었음)
         * 아무 것도 읽지 못하고 넘어간 뒤 다음 출력구문을 실행시킨 것이다.
         *  이걸 해결하기 위해서는 sc.nextLine() 호출 전에 미리 sc.nextLine()을 한 번 호출 하면
         *  그 행을 읽어드리는걸 다음 행으로 넘기게 되고, 그 이후 새로 입력받은 한 라인을 읽어와서
         * str2에 저장하게 되므로 정상 동작하게 된다.
         * */
    }
}
